...
router.post('/', function (req, res, next) {
  ...
  if (!title || !price || !description || !category || !images) {
    errors.push("Các trường không được để trống");
  }
...
router.get('/', function (req, res, next) {
  ...
  let maxPrice = queries.maxPrice ? Number(queries.maxPrice) : 1E6;
  let page = queries.page ? Number(queries.page) : 1;
  let limit = queries.limit ? Number(queries.limit) : 10;
  let errors = [];
  ...
  if (!isNaN(minPrice) && !isNaN(maxPrice) && Number(maxPrice) < Number(minPrice)) {
